...
**Status:** ACTIVE
**Release line:** 0.2.0 candidate / not published
**Current phase:** REL-0003-02 — Select Version and Prepare Release Candidate
...
| REL-0003 Next Release Line | IN PROGRESS | Version `0.2.0` is explicitly approved for candidate preparation. No `v0.2.0` tag, GitHub Release, asset upload, or PyPI publication has occurred. |
...
| Quality gates | PASS / FRESH CANDIDATE EVIDENCE PENDING | Pre-candidate Linux baseline: pytest 155 passed, Ruff 0, mypy 0 in 79 source files. Pre-candidate Windows evidence: pytest 153 passed / 2 permitted symlink-fixture skips, Ruff 0, mypy 0 in 79 source files. |

## Current Priorities

1. Prepare the approved `0.2.0` package/release candidate without creating a public tag or release.
2. Regenerate and verify version-consistent package metadata, lockfile, artifacts, installed metadata, and MCP server version.
3. Gather fresh Linux CI and Windows-local evidence for the exact candidate state.
4. Preserve SAFE-0002 and AUTO-0002 boundaries exactly; release preparation must not add functionality.
5. Keep PyPI explicitly not approved/not published.
6. Stop for separate explicit approval before any `v0.2.0` tag, GitHub Release, or release-asset upload.

## Implemented Baseline

The candidate baseline includes the official Python MCP SDK boundary, 15 registered tool operations, SDK project creation/scaffold CLI, AUTO-0001 project bootstrap, AUTO-0002 deterministic documentation synchronization, local wheel/sdist verification, automated GitHub Actions quality gates, SAFE-0001 Workspace path authorization, and SAFE-0002 active-MCP Git/Python execution safety.
...
REL-0003 has selected version `0.2.0` for candidate preparation. This selection authorizes version/readiness changes only. It does not authorize publication. The exact candidate SHA will be frozen only after the version-consistent preparation changes are merged to canonical `master` and fresh evidence is green.

Git tag `v0.1.0` and GitHub Release `AI-Engineering 0.1.0` remain immutable historical publication evidence for commit `73929bd15fa7637db8162aac199697582bb25e67`. AUTO-0001, AUTO-0002, SAFE-0002, and 0.2.0 candidate preparation are not retroactively part of the published v0.1.0 artifact. PyPI remains not approved and not published.

## Active Work

REL-0003-02 is active. MCP-0002, MCP-0003, SDK-0001 V1/1.1/1.2, TOOL-0001, REL-0001, CI-0001, SAFE-0001, SAFE-0002, REL-0002, AUTO-0001, and AUTO-0002 remain complete and verified for their approved scopes.

## Planned Work

After candidate preparation: freeze the exact `master` SHA, collect fresh Linux/Windows/distribution evidence, prepare the release-readiness record and release notes, and request separate publication approval. Marker initialization, broader engineering automation, additional client/IDE interoperability, additional bootstrap profiles, future execution/security boundaries for new tools, and PyPI remain separately scoped future work.
